...
bool DAQ_CreateAnnieEvent::Execute(){
	...
	if(m_data->TCS.receiveFlag == 1)
	{
		...
		m_data->Stores["ANNIEEvent"]->Set(WaveformLabel,LAPPDWaveforms);
		//m_data->Stores["LAPPD"]->Set(AccLabel,m_data->TCS.ParsedAccStream);
		m_data->Stores["ANNIEEvent"]->Set(MetaLabel,m_data->TCS.ParsedMetaStream);
		//m_data->Stores["LAPPD"]->Set(PPSLabel,m_data->TCS.ParsedPpsStream);
		m_data->Stores["ANNIEEvent"]->Save(path.c_str()); std::cout << "SAVED" << std::endl;	
